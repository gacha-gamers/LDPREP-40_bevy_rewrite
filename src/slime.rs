use crate::consts::*;
use crate::{
    global::{AnimatedSprite, GameLayer, Handles, Moves},
    player::{Player, SlimeThrowEvent},
    GameStates,
};
    ...
    fn build(&self, app: &mut App) {
        app.add_system_set(
            SystemSet::on_update(GameStates::Game)
                ...
                .with_system(spawn_slime)
                .with_system(slime_follow)
                .with_system(slime_animation)
                .with_system(slime_throw),
        )
...
fn slime_spawner(
    ...
    for _ in event.iter() {
        ...
        commands.spawn((
            ...
            AnimatedSprite {
                timer: Timer::from_seconds(1. / SLIME_ANIMATION_FPS, TimerMode::Repeating),
            },
            Moves::default(),
            Slime {
...
fn spawn_slime(input: Res<Input<KeyCode>>, mut event: EventWriter<SpawnSlimeEvent>) {
...
fn slime_follow(
    ...
    for (slime_entity, mut slime_tf, slime) in slime_query.iter_mut() {
        ...
        let previous_entity = follow_order[follow_order
            .iter()
            .position(|x| *x == slime_entity)
            .unwrap()
            - 1];

        let position_to_lerp = positions_hash_map.get(&previous_entity).unwrap();

        if (slime_tf.translation - *position_to_lerp).length() < SLIME_PADDING {
            continue;
        }

        slime_tf.translation =
            slime_tf.translation + (*position_to_lerp - slime_tf.translation) * SLIME_FOLLOW_WEIGHT;
    }
}
...
fn slime_animation(
    player_query: Query<
        ...
        (
            Or<(Changed<Handle<TextureAtlas>>, Changed<TextureAtlasSprite>)>,
            With<Player>,
            Without<Slime>,
        ),
    >,
    mut slime_query: Query<
        (&Slime, &mut Handle<TextureAtlas>, &mut TextureAtlasSprite),
    ...
    for (slime, mut slime_handle, mut slime_sprite) in slime_query.iter_mut() {
        if slime.following_player {
            *slime_handle = player_handle.clone();
            slime_sprite.flip_x = player_sprite.flip_x;
        } else {
            *slime_handle = assets.slime_flying.clone();
        }
    }
}
